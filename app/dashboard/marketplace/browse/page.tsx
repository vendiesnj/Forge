    ...
    >
      ...
      {listing.tech_stack?.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          ...
          {listing.tech_stack.length > 5 && <span className="text-[10px] px-1.5 py-0.5 text-ink4">+{listing.tech_stack.length - 5}</span>}
        </div>
      )}
...
export default function BrowseMarketplacePage() {
  ...
  return (
    <>
      <Topbar title="Forge Marketplace" subtitle="Discover apps from the community" />
      <div className="p-5">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            {!search && filter === 'all' && (
              <Link href="/dashboard/marketplace" className="px-4 py-2 bg-ink text-white text-xs font-medium rounded-forge hover:bg-ink2 transition-colors">
                List your app →
              </Link>
            )}
          </div>
